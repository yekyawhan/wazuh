...
param(
    ...
    [string]$LocalRulesRepo       = "",                    # path to local clone of suricata-win-offline repo. When set, agb-* rules and AR scripts are read from disk instead of GitHub. Empty = GitHub (default).
...
# Auto-set -LocalRulesRepo from the script's own directory when run from the
# Wazuh shared folder (so no GitHub pull is needed and no param is required).
# -LocalRulesRepo on the command line still overrides this.
# Must stay BELOW the Log/Warn/Die definitions: PowerShell only knows a
# function once execution has passed its definition, so calling Log() from
# above them fails with "The term 'Log' is not recognized".
if (-not $LocalRulesRepo -and $PSScriptRoot) {
    $candidate = $PSScriptRoot.TrimEnd('\')
    if (Test-Path (Join-Path $candidate "rules\agb-black.rules")) {
        $LocalRulesRepo = $candidate
        Log "LocalRulesRepo auto-detected from script location: $LocalRulesRepo"
    }
}
...
function Get-AgbBlackDropRuleset([string]$destPath) {
    ...
    try {
        if ($LocalRulesRepo) {
            $localPath = Join-Path $LocalRulesRepo "rules\agb-black.rules"
            if (Test-Path $localPath) {
                $text = [IO.File]::ReadAllText($localPath)
                Log "  agb-black.rules read from local: $localPath"
            } else {
                Warn "  local path $localPath not found"
                return $false
            }
        } else {
...
if (-not $SkipRulesSetup) {
    ...
    if (-not $yamlSrc) {
        ...
        try {
            if ($LocalRulesRepo) {
                $localPath = Join-Path $LocalRulesRepo "rules\agb-white.rules"
                if (Test-Path $localPath) {
                    Copy-Item $localPath "$RuleDir\agb-white.rules" -Force
                    $passCount = ([regex]::Matches([IO.File]::ReadAllText("$RuleDir\agb-white.rules"), '(?m)^\s*pass\s')).Count
                    Log "  agb-white.rules copied from local: $localPath ($passCount pass signatures)"
                } else { Warn "  local path $localPath not found" }
            } else {
        ...
        try {
            if ($LocalRulesRepo) {
                $localPath = Join-Path $LocalRulesRepo "rules\agb-heuristics.rules"
                if (Test-Path $localPath) {
                    Copy-Item $localPath "$RuleDir\agb-heuristics.rules" -Force
                    Log "  agb-heuristics.rules copied from local: $localPath"
                } else { Warn "  local path $localPath not found" }
            } else {
...
# calls .Replace('__LOCALRULESREPO__', ...) and Copy-LocalRulesFile reads
...
$LocalRulesRepo = '__LOCALRULESREPO__'
...
'@
    ...
    $refreshBody = $refreshTemplate.
        ...
        Replace('__LOCALRULESREPO__', $(if ($LocalRulesRepo) { $LocalRulesRepo } else { '' })).
    ...
    $rTrigger   = New-ScheduledTaskTrigger -Once -At (Get-Date) -RepetitionInterval (New-TimeSpan -Hours 3) -RepetitionDuration (New-TimeSpan -Days 3650)
...
if ($SkipWazuhWiring) {
    ...
    if (Test-Path $arBinDir) {
        ...
        try {
            $arFiles = @('agb-kill-block.ps1', 'agb-kill-block.cmd')
            foreach ($arFile in $arFiles) {
                if ($LocalRulesRepo) {
                    $localPath = Join-Path $LocalRulesRepo "wazuh-manager\active-response\$arFile"
                    if (Test-Path $localPath) {
                        Copy-Item $localPath "$arBinDir\$arFile" -Force
                        Log "  $arFile copied from local: $localPath"
                    } else { throw "$arFile not found at local path: $localPath" }
                } else {
                    Invoke-WebRequest -Uri "$RepoRawBase/wazuh-manager/active-response/$arFile" -OutFile "$arBinDir\$arFile" -UseBasicParsing
                }
            }
            # SECURITY: log the SHA256 of what was actually deployed
...
if ($SkipService) {
    ...
    if (-not $svcConfirmed) {
        ...
        # "Loading signatures failed" is the SUMMARY line those file.magic
        # errors produce, not an independent fault - without filtering it too,
        # $realErrors is never empty and this gate refuses to install the
        # service on a perfectly good config. A genuine problem still emits its
        # own specific E: line, which is what actually trips the gate.
        $errLines   = @($testOut | Where-Object { "$_" -match '^E:' })
        $realErrors = @($errLines | Where-Object { "$_" -notmatch "file\.magic" -and "$_" -notmatch 'Loading signatures failed' })
        if ($realErrors.Count -gt 0) {
