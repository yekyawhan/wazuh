...
$ok3 = Test-Path "$psDir\install-watcher.ps1"
Write-Host ""
Write-Host ("  {0}  {1}\reenable-defender.cmd" -f $(if($ok1){"[OK]"}else{"[!!]"}), $binDir)
Write-Host ("  {0}  {1}\reenable-defender.ps1" -f $(if($ok2){"[OK]"}else{"[!!]"}), $psDir)
Write-Host ("  {0}  {1}\install-watcher.ps1"  -f $(if($ok3){"[OK]"}else{"[!!]"}), $psDir)

if ($ok1 -and $ok2 -and $ok3) {
    Write-Host "`nAgent side installed." -ForegroundColor Green
    Write-Host "Next:" -ForegroundColor Cyan
    Write-Host "  1. (recommended) Register instant watcher — fires on FIRST disable, no manager round-trip:" -ForegroundColor Cyan
